...
export async function POST(request: Request) {
  ...
  const { data: patient } = await admin
    .from("ai_patients")
    .select(`name, interaction_mode, ${PATIENT_PROMPT_COLUMNS}`)
    .eq("id", patientId)
    .single();

  if (!patient) return NextResponse.json({ error: "Paciente no encontrado" }, { status: 404 });

  // Piloto de voz (AU-03/AC-06): sin interrupciones de texto en la
  // variante voice_only — las interrupciones ahí las maneja el relé de voz.
...
El terapeuta lleva un rato escribiendo algo pero no lo envía. Reacciona naturalmente al verlo escribir sin enviar. Opciones según tu personalidad:
- "¿Va a enviar algo o solo escribe?" [mira con curiosidad]
- "Parece que le cuesta encontrar las palabras..."
...
Ha pasado más de un minuto sin que el terapeuta escriba nada. Reacciona al silencio corto según tu personalidad:
- Mostrar incomodidad: [se mueve en la silla] "Este silencio me incomoda un poco..."
- Llenar el vacío: "Bueno... no sé si debería seguir escribiendo o..."
- Mostrar nerviosismo: [juega con las manos] "¿Está esperando que escriba algo más?"
...
Han pasado unos 3 minutos sin que el terapeuta escriba. El silencio se ha extendido. Reacciona con más intensidad:
- Frustración: "¿Me está leyendo? Porque no ha respondido nada..."
- Vulnerabilidad: "Este silencio me hace pensar que escribí algo malo..."
