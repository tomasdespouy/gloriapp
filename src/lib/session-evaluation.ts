 ...
 * aviso al docente. Reusado por el cron de barrido y por reeval-session.
 *  - Si ya hay una eval APROBADA/publicada por el docente, NO la pisa (skip).
 *  - NO retrotrae la gamificación (XP/racha): es estado "vivo" ligado a la fecha
 *    y al orden de las sesiones; recuperarla en diferido corrompería la racha y
 *    podría doblar XP/correos en carrera con /complete. Se acepta que una sesión
 *    rescatada (raro con jsonMode) no sume XP — lo clínico (eval+resumen+aviso) sí.
